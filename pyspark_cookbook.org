...
#+begin_src python :var data="" :exports none :results output :session none :exports none
  ...
  for j, line in enumerate(data.split("\n")):
      # Append and prepend white spaces to square brackets to prevent Org-Mode
...
#+BEGIN_SRC python :post pretty2orgtbl(data=*this*)
  ...
  window_spec = Window.partitionBy("user").orderBy("tstp_unix")
...
#+BEGIN_SRC python :post pretty2orgtbl(data=*this*)
  ...
  def arr_intxn(v: pd.Series) -> List[str]:
      """
  ...
  w = Window.partitionBy("group").orderBy("letters").rowsBetween(-1, 0)
  df = df.withColumn("nbr_of_rows_in_input_win", number_of_input_rows_in_win("letters").over(w))
  df = df.withColumn("intxn_with_prev_row", arr_intxn("letters").over(w))
  df = df.withColumn("size_of_intxn_with_prev_row", arr_intxn_size("letters").over(w))
...
|  group|     letters|nbr_of_rows_in_input_win|intxn_with_prev_row|size_of_intxn_with_prev_row|
|-------+------------+------------------------+-------------------+---------------------------|
|group_A|   [a, b, c]|                       1|                 []|                          0|
|group_A|[a, x, z, w]|                       2|                [a]|                          1|
|group_A|[a, x, z, y]|                       2|          [a, x, z]|                          3|
|group_A|   [d, x, z]|                       2|             [x, z]|                          2|
|group_B|   [a, c, d]|                       1|                 []|                          0|
|group_B|   [a, c, e]|                       2|             [c, a]|                          2|
|group_B|[a, g, f, h]|                       2|                [a]|                          1|
:end:
