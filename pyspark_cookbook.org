...
#+BEGIN_SRC python :post pretty2orgtbl(data=*this*)
  ...
  def aggregate_distribution(df_stat, col, count_column, window=None, to_order_by_pct=True, n=None, operations=None, op_cols=None):
      ...
      df_stat_agg = df_stat_agg.withColumn(
          ...
          F.round(
              100 * F.col(f"n_of_{count_column}s_having")
  ...
  txt = "Group by \"group\" field only:"
  <<txtblk("txt")>>print(txt)
  df_stat = aggregate_distribution(df, ["group"], "student_id", window=None, to_order_by_pct=False)
...
#+begin_src text
Group by "group" field only:
#+end_src

|%_of_student_ids_having|n_of_student_ids_having|group|n_of_student_ids_in_win|
|-----------------------+-----------------------+-----+-----------------------|
|46.0                   |46                     |1    |100                    |
|41.0                   |41                     |2    |100                    |
|12.0                   |12                     |3    |100                    |
...
#+BEGIN_SRC python :post pretty2orgtbl(data=*this*)
  ...
  <<txtblkend>>
  # Alternatively, use matplotlib.pyplot:
  df_hist.toPandas().plot(x="value", y="count", kind="bar", rot=90).grid(axis='y')
  <<plt_show("images/histogram_one_dimensional.png")>>plt.show()
#+END_SRC
...
0.975 |  1
#+end_src
[[file:images/histogram_one_dimensional.png]]
:end:
